...
<head>
  ...
  <body class="app sidebar-mini rtl">
    <main class="app-content">
      <div class="app-title">
        <div>
          <h1><i class="fa fa-th-list"></i>单选题</h1>
        </div>
        <ul class="app-breadcrumb breadcrumb">
          <li class="breadcrumb-item"><i class="fa fa-home fa-lg"></i></li>
          <li class="breadcrumb-item">教师</li>
          <li class="breadcrumb-item active"><a href="#">单选题</a></li>
        </ul>
      </div>
      <div class="row">
        ...
        <div class="col-md-12">
          <div class="tile">
           ...
           <div class="tile-body">
              <form class="row" action="${basePath }teacher/single" method="post">
            ...
            <table class="table table-bordered">
              ...
              <tbody align="center">
              	<c:forEach items="${pageInfo.list }" var="single" varStatus="status">
                <tr id="tr${single.id }">                  
                  <td>${status.count }</td>
                  <td>${single.title}</td>
                  <td>${single.qtype }</td>
                  ...
                  <td>
                  	<button class="btn btn-info" type="button" style="height:30px;line-height:10px;" onclick="edit('${single.id}')">编辑</button>
                  	<button class="btn btn-danger" type="button" style="height:30px;line-height:10px;" onclick="del('${single.id}')">删除</button>
                  </td>
                </tr>
              	</c:forEach>
    ...
    <script type="text/javascript">
    	...
    	function del(sid){
    		layer.confirm('您确定要删除吗？',
    			...
    			function(){//确定
    				$.ajax({
    					type:'post',
    					url:'${basePath}teacher/delSingle/'+sid,//restFull
    	...
    	function add(){
	    	layer.open({
	    		...
	    		title:'添加单选题',
	    		area:['550px','550px'],
	    		content:'${basePath}teacher/toAddSingle',
	    		btn:'添加',
	    		skin:'my-skin',
        ...
        function edit(sid){
          layer.open({
            ...
            title:'编辑单选题',
            area:['550px','550px'],
            content:'${basePath}teacher/toEditSingle/'+sid,
            btn:'修改',
            skin:'my-skin',
            yes:function(index,layero){
              //调用弹出层页面js
