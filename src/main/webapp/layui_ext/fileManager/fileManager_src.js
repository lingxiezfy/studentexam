layui.define(['jquery', 'layer', 'laypage'], function(exports) { //提示：模块也可以依赖其它模块，如：layui.define('layer', callback);
    ...
    //外部接口
    var fm = {
        config:{test:'test',thumb:{'nopic':'ico/null-100x100.jpg',width:100,height:100},icon_url:'ico/',btn_upload:true,btn_create:true,file_base: ''}
        ,cache: {} //数据缓存
    ...
    Class.prototype.renderData = function(res, curr, count){
        ...
        layui.each(data,function(i,v){
            ...
            switch (v.type) {
                case 'directory':
                    _img = '<div  style="width:' + options.thumb['width'] + 'px;height:' + options.thumb['height'] + 'px;line-height:' + options.thumb['height'] + 'px"><img src="' + options.file_base + options.icon_url + 'dir.png" style="vertical-align:middle;"></div>';
                    _type = 'DIR';
                    break;
                default:
                    
                    if (v.type === 'png' || v.type === 'gif' || v.type === 'jpg' || v.type === 'image') {
                        _img = '<img src="' + v.thumb + '" width="'+options.thumb['width']+'" height="'+options.thumb['height']+'" onerror=\'this.src="' +options.file_base+options.thumb['nopic']+'"\'  />';
                    } else {
                        _img = '<div  style="width:' + options.thumb['width'] + 'px;height:' + options.thumb['height'] + 'px;line-height:' + options.thumb['height'] + 'px"><img src="' + options.file_base + options.icon_url +'/'+ v.type + '.png"  onerror=\'this.src="' + options.file_base + options.thumb['nopic'] + '"\' /></div>';
                    }
                    break;
            }
    ...
    Class.prototype.events = function(){
        ...
        that.layBody.on('click', 'li', function(){ //单击行
            setPicEvent.call(this, 'fileClick');
        });
        //右击事件
        that.layBody.on('contextmenu', 'li', function(e){
            e.preventDefault();
            ...
            layui.event.call(this,
                ...
                ,{obj:othis,data:data,parent:fm.dirRoot[fm.dirRoot.length -1]['path']}
            );
        });
        //文件夹单击事件
